...
function runDoctor(cwd) {
  ...
  const statePath = path.join(terraceDir, 'project-state.json');
  ...
  if (fs.existsSync(terraceDir) && !fs.existsSync(statePath)) {
    blocking.push({
      ...
      message: 'Missing .terrace/project-state.json',
      remediation: 'Re-run `terrace init` to create project-state.json.'
    });
  }
